...
#+SUBTITLE: for version 2.90.1 (v2.90.1-1126-g1a89a8474+1)
...
#+TEXINFO: @noindent
This manual is for Magit version 2.90.1 (v2.90.1-1126-g1a89a8474+1).
...
- Function: magit-run-git-with-logfile file &rest args
  ...
  Calls git synchronously with ARGS.  The process's output is saved in
...
With emacs versions after 26.1 you may set ~--alternate-editor~ to the
empty string.  Pay attention to the use of single and double quotes as
well as the use of the `=` sign when setting the `EDITOR` variable.

#+BEGIN_SRC shell-script
  $ killall emacs
  $ EDITOR='emacsclient --alternate-editor="" --create-frame' git commit
#+END_SRC

Or in git config like this:

#+BEGIN_SRC shell-script
  $ killall emacs
  $ git config --global core.editor "emacsclient --alternate-editor='' --create-frame"
  $ git commit
#+END_SRC

Replace `--create-frame` with `--tty`  if you would like to now open
a graphical frame and stay in your tty.
